...
Run from project root. Idempotent: skips if Week 1 point already in dataset.
"""
import sys
...
def main():
    ...
    for fid in range(1, 9):
        ...
        if current is not None:
            ...
            if _already_appended(X_cur, x_new):
                print(f"Function {fid}: already appended (Week 1 in dataset), skip. Points: {len(y_cur)}")
                continue
    ...
    print("Done. data/problems/function_1..8 updated (CSV). Re-run notebooks to use appended points.")
